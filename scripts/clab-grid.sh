...
uv run setup/inject_functional.py  clab-ospfv3-grid10x10 --max-executions 2 -t link --min-interval 20 --max-interval 30 --specific-link 3,2-4,2 --execute

uv run setup/inject.py  clab-ospfv3-grid10x10 --max-executions 2 -t link --min-interval 20 --max-interval 30 

uv run auto.py clab-ospfv3-grid10x10 grid-collect
